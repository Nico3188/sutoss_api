...
namespace Sutoss.Controllers
{
    ...
    public class AlquilersController : ControllerBase
    {
        ...
        private readonly IAlquilersService _service;

        public AlquilersController(ILogger<AlquilersController> logger, IAlquilersService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(AlquilerRequest newAlquiler)
        {
            try 
            {
                return Ok(await _service.Create(newAlquiler  ));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(AlquilerRequest Alquiler)
        {
            try
            {
                return Ok(await _service.Update(Alquiler  ));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
