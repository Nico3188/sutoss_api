...
namespace Sutoss.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class CelebracionsController : ControllerBase
    {
        private readonly ILogger<CelebracionsController> _logger;
        private readonly ICelebracionsService _service;

        public CelebracionsController(ILogger<CelebracionsController> logger, ICelebracionsService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        public async Task<IActionResult> Create(CelebracionRequest newCelebracion)
        {
            try 
            {
                return Ok(await _service.Create(newCelebracion  ));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        public async Task<IActionResult> Update(CelebracionRequest Celebracion)
        {
            try
            {
                return Ok(await _service.Update(Celebracion  ));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
