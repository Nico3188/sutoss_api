...
namespace sutoss.Controllers
{
    ...
    public class beneficiosController : ControllerBase
    {
        ...
        public beneficiosController(ILogger<beneficiosController> logger, IbeneficiosService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(beneficioRequest newbeneficio)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newbeneficio, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(beneficioRequest beneficio)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(beneficio, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
