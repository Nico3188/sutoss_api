...
namespace sutoss.Controllers
{
    ...
    public class suscripcionesController : ControllerBase
    {
        ...
        private readonly IsuscripcionesService _service;

        public suscripcionesController(ILogger<suscripcionesController> logger, IsuscripcionesService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(suscripcionRequest newsuscripcion)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newsuscripcion, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(suscripcionRequest suscripcion)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(suscripcion, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
