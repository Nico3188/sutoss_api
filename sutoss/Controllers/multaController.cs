...
namespace sutoss.Controllers
{
    ...
    public class multasController : ControllerBase
    {
        ...
        private readonly ImultasService _service;

        public multasController(ILogger<multasController> logger, ImultasService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(multaRequest newmulta)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newmulta, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(multaRequest multa)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(multa, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
