...
namespace sutoss.Controllers
{
    ...
    public class mantenimientosController : ControllerBase
    {
        ...
        public mantenimientosController(ILogger<mantenimientosController> logger, ImantenimientosService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(mantenimientoRequest newmantenimiento)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newmantenimiento, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(mantenimientoRequest mantenimiento)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(mantenimiento, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
