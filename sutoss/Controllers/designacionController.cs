...
namespace sutoss.Controllers
{
    ...
    {
        private readonly ILogger<designacionesController> _logger;
        private readonly IdesignacionesService _service;

        public designacionesController(ILogger<designacionesController> logger, IdesignacionesService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        public async Task<IActionResult> Create(designacionRequest newdesignacion)
        {
            try 
            {
                ...
                return Ok(await _service.Create(newdesignacion, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        public async Task<IActionResult> Update(designacionRequest designacion)
        {
            ...
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(designacion, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
