...
namespace sutoss.Controllers
{
    ...
    public class cuotasController : ControllerBase
    {
        ...
        private readonly IcuotasService _service;

        public cuotasController(ILogger<cuotasController> logger, IcuotasService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(cuotaRequest newcuota)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newcuota, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(cuotaRequest cuota)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(cuota, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
