...
namespace sutoss.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class pretamosController : ControllerBase
    {
        private readonly ILogger<pretamosController> _logger;
        private readonly IpretamosService _service;

        public pretamosController(ILogger<pretamosController> logger, IpretamosService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        public async Task<IActionResult> Create(prestamoRequest newprestamo)
        {
            try 
            {
                ...
                return Ok(await _service.Create(newprestamo, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(prestamoRequest prestamo)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(prestamo, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
