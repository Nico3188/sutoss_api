...
namespace sutoss.Controllers
{
    ...
    public class premiosController : ControllerBase
    {
        ...
        public premiosController(ILogger<premiosController> logger, IpremiosService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(premioRequest newpremio)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newpremio, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(premioRequest premio)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(premio, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
