...
namespace sutoss.Controllers
{
    ...
    public class serviciosController : ControllerBase
    {
        ...
        public serviciosController(ILogger<serviciosController> logger, IserviciosService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(servicioRequest newservicio)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newservicio, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(servicioRequest servicio)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(servicio, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
