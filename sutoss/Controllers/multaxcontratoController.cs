...
namespace sutoss.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class multasxcontradoController : ControllerBase
    {
        private readonly ILogger<multasxcontradoController> _logger;
        private readonly ImultasxcontradoService _service;

        public multasxcontradoController(ILogger<multasxcontradoController> logger, ImultasxcontradoService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        public async Task<IActionResult> Create(multaxcontratoRequest newmultaxcontrato)
        {
            try 
            {
                ...
                return Ok(await _service.Create(newmultaxcontrato, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        public async Task<IActionResult> Update(multaxcontratoRequest multaxcontrato)
        {
            ...
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(multaxcontrato, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
