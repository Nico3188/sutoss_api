...
namespace sutoss.Controllers
{
    ...
    public class facturasController : ControllerBase
    {
        ...
        public facturasController(ILogger<facturasController> logger, IfacturasService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(facturaRequest newfactura)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newfactura, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(facturaRequest factura)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(factura, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
