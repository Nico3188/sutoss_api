...
namespace sutoss.Controllers
{
    ...
    public class anticiposController : ControllerBase
    {
        ...
        private readonly IanticiposService _service;

        public anticiposController(ILogger<anticiposController> logger, IanticiposService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        public async Task<IActionResult> Create(anticipoRequest newanticipo)
        {
            try 
            {
                ...
                return Ok(await _service.Create(newanticipo, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        public async Task<IActionResult> Update(anticipoRequest anticipo)
        {
            ...
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(anticipo, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
