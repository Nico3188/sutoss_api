...
namespace sutoss.Controllers
{
    ...
    public class horariosController : ControllerBase
    {
        ...
        public horariosController(ILogger<horariosController> logger, IhorariosService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(horarioRequest newhorario)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newhorario, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(horarioRequest horario)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(horario, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
