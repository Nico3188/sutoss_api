...
namespace sutoss.Controllers
{
    ...
    public class diasController : ControllerBase
    {
        ...
        private readonly IdiasService _service;

        public diasController(ILogger<diasController> logger, IdiasService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(diaRequest newdia)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newdia, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(diaRequest dia)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(dia, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
