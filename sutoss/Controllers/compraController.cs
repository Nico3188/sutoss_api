...
namespace sutoss.Controllers
{
    ...
    public class comprasController : ControllerBase
    {
        ...
        public comprasController(ILogger<comprasController> logger, IcomprasService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(compraRequest newcompra)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newcompra, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(compraRequest compra)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(compra, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
