...
namespace sutoss.Controllers
{
    ...
    public class provinciasController : ControllerBase
    {
        ...
        public provinciasController(ILogger<provinciasController> logger, IprovinciasService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(provinciaRequest newprovincia)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newprovincia, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(provinciaRequest provincia)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(provincia, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
