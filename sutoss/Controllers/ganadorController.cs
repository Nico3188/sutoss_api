...
namespace sutoss.Controllers
{
    ...
    {
        private readonly ILogger<ganadoresController> _logger;
        private readonly IganadoresService _service;

        public ganadoresController(ILogger<ganadoresController> logger, IganadoresService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        public async Task<IActionResult> Create(ganadorRequest newganador)
        {
            try 
            {
                ...
                return Ok(await _service.Create(newganador, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        public async Task<IActionResult> Update(ganadorRequest ganador)
        {
            ...
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(ganador, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
