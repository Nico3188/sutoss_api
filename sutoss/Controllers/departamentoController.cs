...
namespace sutoss.Controllers
{
    ...
    public class departamentosController : ControllerBase
    {
        ...
        public departamentosController(ILogger<departamentosController> logger, IdepartamentosService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(departamentoRequest newdepartamento)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newdepartamento, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(departamentoRequest departamento)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(departamento, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
