...
namespace sutoss.Controllers
{
    ...
    {
        private readonly ILogger<vinculosController> _logger;
        private readonly IvinculosService _service;

        public vinculosController(ILogger<vinculosController> logger, IvinculosService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(vinculoRequest newvinculo)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newvinculo, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(vinculoRequest vinculo)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(vinculo, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
