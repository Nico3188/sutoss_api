...
namespace sutoss.Controllers
{
    ...
    public class conveniosController : ControllerBase
    {
        ...
        private readonly IconveniosService _service;

        public conveniosController(ILogger<conveniosController> logger, IconveniosService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(convenioRequest newconvenio)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newconvenio, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(convenioRequest convenio)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(convenio, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
