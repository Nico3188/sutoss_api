...
namespace sutoss.Controllers
{
    ...
    public class familiasController : ControllerBase
    {
        ...
        private readonly IfamiliasService _service;

        public familiasController(ILogger<familiasController> logger, IfamiliasService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(familiaRequest newfamilia)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newfamilia, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(familiaRequest familia)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(familia, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
