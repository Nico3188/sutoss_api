...
namespace sutoss.Controllers
{
    ...
    public class eventosController : ControllerBase
    {
        ...
        public eventosController(ILogger<eventosController> logger, IeventosService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(eventoRequest newevento)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newevento, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(eventoRequest evento)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(evento, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
