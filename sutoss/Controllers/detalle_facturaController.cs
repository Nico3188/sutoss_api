...
namespace sutoss.Controllers
{
    ...
    {
        private readonly ILogger<detalle_facturasController> _logger;
        private readonly Idetalle_facturasService _service;

        public detalle_facturasController(ILogger<detalle_facturasController> logger, Idetalle_facturasService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(detalle_facturaRequest newdetalle_factura)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newdetalle_factura, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(detalle_facturaRequest detalle_factura)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(detalle_factura, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
