...
namespace sutoss.Controllers
{
    ...
    public class perfilesController : ControllerBase
    {
        ...
        public perfilesController(ILogger<perfilesController> logger, IperfilesService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        [HttpPost("Create")]
        [Authorize]
        public async Task<IActionResult> Create(perfilRequest newperfil)
        {
            try 
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Create(newperfil, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        [HttpPut("Update")]
        [Authorize]
        public async Task<IActionResult> Update(perfilRequest perfil)
        {
            try
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(perfil, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
