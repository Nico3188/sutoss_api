...
namespace sutoss.Controllers
{
    ...
    public class instalacionesController : ControllerBase
    {
        ...
        private readonly IinstalacionesService _service;

        public instalacionesController(ILogger<instalacionesController> logger, IinstalacionesService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        public async Task<IActionResult> Create(instalacionRequest newinstalacion)
        {
            try 
            {
                ...
                return Ok(await _service.Create(newinstalacion, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
        ...
        public async Task<IActionResult> Update(instalacionRequest instalacion)
        {
            ...
            {
                string userId = KeyCloak.KeyCloakManager.GetExternalUserId(User);
                return Ok(await _service.Update(instalacion, userId));
            }
            catch (NotFoundException ex)
            {
                // Logger.Warn(ex);
